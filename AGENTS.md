...
- Typography uses Source Serif 4 for headings and Outfit for body text via `next/font/google` in `app/layout.tsx`; `config/typography.ts` documents the pair for Tailwind (`font-heading`, default `font-sans`).
